...
根据群友的说法，校园流量套餐大约为 59 元/月，内含 100G 通用流量、80G 专属流量（部分 APP 可用）、500 分钟通话时长、500 条短信。另叠加 10 元/月（优惠前 20 元/月）的福州高校宽带电视融合套餐，即 100Mbps、限 2 个设备同时在线的 [校园网](#campus-network)。可能有合约（例如未使用到一定时长销户时需要交补偿金），办理时还请注意。
